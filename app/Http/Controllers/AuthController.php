...
class AuthController extends Controller
{
    ...
    public function register($form=1, $type='')
    {
        ...
        if( Input::get('_token') ) {
            ...
            if( Input::get('user_type_provider') == 'owner' ) {
                unset($user_data['provider_type']);
            } else {
                $user_data['minimum_fee'] = 0;                
            }
            ...
            */ 
            ...
            $u->fullname    = Input::get('firstname').' '.Input::get('lastname');

            $invitation_code    = Input::get('invitation_code');
            $user_type_provider = $user_data['user_type_provider'];      

            $unsets = ['_token', 'user_type_provider', 'password', 'password_confirmation', 'agree', 'email'];
