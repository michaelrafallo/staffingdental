...
<div class="tabbable-custom nav-justified">
    <ul class="nav nav-tabs nav-justified hidden-xs"">
        <li class="{{ actived($status, 'pending') }}">
            <a href="{{ URL::route('provider.appointments.index', ['status' => 'pending']) }}"> Pending<br> Appointments 
            @if($pending)
        ...
        <li class="{{ actived($status, 'confirmed') }}">
            <a href="{{ URL::route('provider.appointments.index', ['status' => 'confirmed']) }}"> Accepted<br> Appointments 
            @if($confirmed)
        ...
        <li class="{{ actived($status, 'for_approval') }}">
            <a href="{{ URL::route('provider.appointments.index', ['status' => 'for_approval']) }}"> Completed<br> (For Approval) 
            @if($for_approval)
        ...
        <li class="{{ actived($status, 'completed') }}">
            <a href="{{ URL::route('provider.appointments.index', ['status' => 'completed']) }}"> Completed<br> Appointments
            @if($completed)
        ...
        <li class="{{ actived($status, 'cancelled') }}">
            <a href="{{ URL::route('provider.appointments.index', ['status' => 'cancelled']) }}"> Cancelled<br> Appointments
            @if($cancelled)
    ...
    <div class="tab-content">
        <div class="tab-pane active" id="tab_1_1_1">
            ...
            <h3>{{ appointment_status($status) }}</h3>
             ...
             <table class="table table-striped table-bordered table-hover datatable">
                ...
                <tbody>
                    ...
                    <tr class="{{ ($row->id == Input::get('id')) ? 'actived' : '' }}">
                        <td>{{ $row->id }}</td>
                        <td>
                        <a href="{{ URL::route('provider.job-postings.employer', $row->post_author) }}"  class="uppercase">
                        ...
                        <td>{{ breaktime($postmeta->breaktime) }}</td>
                        <td>
                        {{ $postmeta->total_time }}             
                        </td>
                        ...
                        <td>
                            <a data-href="{{ URL::route('provider.appointments.view', $row->id) }}" 
                            class="btn btn-xs btn-success uppercase btn-view" 
                            data-toggle="modal" 
                            href="#view">View</a>

                            @if($row->post_status == 'pending')
                            <a href="#" class="delete btn btn-xs btn-primary uppercase"
                                data-href="{{ URL::route('provider.appointments.status', ['confirmed', $row->id]) }}" 
                                data-toggle="modal"
                                data-target=".delete-modal" 
                                data-title="Accept Appointment"
                                data-body="Are you sure you want to accept appointment with <b>ID# {{ $row->id }}</b>?">Accept</a>
                            @endif

                            @if(in_array($row->post_status, ['pending', 'confirmed']))
                            <a href="#" class="delete btn btn-xs btn-danger uppercase"
                                data-href="{{ URL::route('provider.appointments.status', ['cancelled', $row->id]) }}" 
                                data-toggle="modal"
                                data-target=".delete-modal" 
