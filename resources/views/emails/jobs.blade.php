...
<p>Here you have {{ $c = count($rows) }} new job{{ is_plural($c) }} that matches your criteria :</p>
...
@foreach($rows as $row)
   ...
   <tr>
      <td colspan="2" style="font-weight: bold;color: #298ed0;">{{ $row->post_title }}</td>         
   </tr>
   <tr>
      <td width="150">Job Type </td>
      <td> : {{ job_type($postmeta->job_type) }}</td>
   </tr>
   <tr>
      <td width="150">Provider Type </td>
      <td> : {{ provider_type($postmeta->provider_type) }}</td>
   </tr>
   <tr>
      <td width="150">Salary </td>
      <td> : {{ number_format($postmeta->salary_rate, 2) }} USD / {{ salary_type($postmeta->salary_type) }}</td>
   </tr>
   <tr>
      <td width="150">Years of experience </td>
      <td> : {{ years_of_experience($postmeta->years_of_experience) }}</td>
   </tr>
   <tr>
      <td width="150">Distance within </td>
      <td> : {{ get_distance($row->distance) }}</td>
   </tr>
   <tr>
   <tr>
      <td colspan="2">
      <a href="{{ URL::route('provider.job-postings.view', $row->id) }}">Apply Now</a>         
      </td>
   </tr>
<td colspan="2" style="border-collapse:collapse;mso-table-lspace:0;mso-table-rspace:0;padding-top:18px;padding-bottom:18px;">
...
<small>This is an automated notification. Please do not reply, as this email account is not monitored.</small>
